...
class Player2(pygame.sprite.Sprite):
    ...
    def collide_obstacle(self, direction):
        if direction == "x":
            collide = pygame.sprite.spritecollide(self, self.obstacles, False)
            if collide:
                if self.vx > 0:
                    self.x = collide[0].rect.left -self.rect.width/2
                if self.vx < 0:
                    self.x = collide[0].rect.right +self.rect.width/2
                self.vel.x = 0
        ...
        if direction == "y":
            ...
            if collide:
                if self.vy > 0:
                    self.y = collide[0].rect.top -self.rect.height/2
                if self.vy < 0:
                    self.y = collide[0].rect.bottom +-self.rect.height/2
                self.vel.y = 0
                self.rect.centery = self.pos.y

    def update(self):
        self.mechanics()
        self.rot = (self.rot + self.rot_speed * clock.tick(FPS)) % 360
        self.image = pygame.transform.rotate(Player2_sprite, self.rot-90)
        self.rect = self.image.get_rect()
        self.rect.center = self.pos
        self.pos += self.vel * clock.tick(FPS)
        self.rect.centerx = self.pos.x
        self.rect.centery = self.pos.y
